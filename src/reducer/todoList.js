...
const todoList =(state =initialState,action ) =>{
    switch (action.type) {
            ...
            case SELECT_ALL:
                ...
                const coppyItem = JSON.parse(localStorage.getItem('jobs'))
                coppyItem.map(a=> a.completed= newState.checkAll)
                localStorage.setItem('jobs',JSON.stringify(coppyItem))
                newState.valueInput=JSON.parse(localStorage.getItem('jobs'))
                })
            case CLEAR_COMPLETED:
                return produce(state, newState=>{
                    const coppyItem = JSON.parse(localStorage.getItem('jobs'))
                    localStorage.setItem('jobs',JSON.stringify(coppyItem.filter(a => a.completed === false)))
                    newState.valueInput=JSON.parse(localStorage.getItem('jobs'))
                    })
            case EDIT:
                return produce(state, newState =>{
            ...
            case SELECTED_ACTIVE:
                return produce(state,newState=>{
                    newState.valueInput=JSON.parse(localStorage.getItem('jobs')).filter(a => a.completed===false)
                })
            case SELECTED_COMPLETED:
                return produce(state,newState=>{
                    newState.valueInput=JSON.parse(localStorage.getItem('jobs')).filter(a => a.completed===true)
                })
        default:
            return state;
    }
