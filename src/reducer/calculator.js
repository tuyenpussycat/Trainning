  ...
  switch (action.type) {
      ...
      return produce(state, newState => {
        if (newState.currentVal.length < 20) {
          if (newState.currentVal.slice(1, 2).includes('.') === false) {
            newState.currentVal.startsWith('0')
              ? (newState.currentVal = newState.currentVal.slice(1) + action.payload)
    ...
    case DOT: {
      return produce(state, newState => {
        if (
          newState.value !== '' &&
          ['+', '-', '/', '*', '0'].includes(newState.formula[newState.formula.length - 1]) === false
        ) {
          newState.formula = 0
          newState.check = true
          newState.currentVal = 0
    ...
    case ZERO: {
      return produce(state, newState => {
        ...
        if (
          newState.value !== '' &&
          ['+', '-', '/', '*', '.'].includes(newState.formula[newState.formula.length - 1]) === false
        ) {
          newState.formula = ''
          newState.formula += action.payload
          newState.check = true
